...
/*
     graph = new Graph(vertexs.size(), distances.size(), distances);

        QVector<long> path, path2;
        graph->TheShortestPathAlg(qrand()%vertexs.size(), qrand()%vertexs.size(), path);
        des = new DES;
        des->DES_Lock(path);
        des->DES_Unlock(path2);
        drawPath(path2);
*/
