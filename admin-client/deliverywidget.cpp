...
//    deliveryTable->horizontalHeader()->setStyleSheet("QHeaderView::section{background:skyblue;}"); //设置表头背景色
    deliveryTable->setItemDelegate(new NoFocusDelegate());  // 消除选中虚框
}
...
void DeliveryWidget::applyPath()
{
    ...
    int index = deliveryTable->currentRow();
    tempPath->vehicle =static_cast<QComboBox*>(deliveryTable->cellWidget(index, 1))->currentText();
    tempPath->cargo = deliveryTable->item(index, 2)->text();

    //信息不完善则弹窗提示
    if(tempPath->vehicle.size() == 0 || tempPath->cargo.size() == 0 ||
